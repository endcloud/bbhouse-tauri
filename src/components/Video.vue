...
// let flv = store.state.settings!.player.flv
...
const danmakuOption = (aid: string, cid: string): DPlayerDanmaku => {
    return {
        id: `${aid}#${cid}`,
        // api: 'https://api.prprpr.me/dplayer/', 'dp.endcloud.cn'
        api: 'http://127.0.0.1:29418/',
        // addition: ["https://dplayer.alone88.cn/v3/bilibili?aid=BV1BZ4y1w77m&cid=313938332"],
        addition: [`http://127.0.0.1:29418/v3/bilibili?aid=${aid}&cid=${cid}`],
    ...
    const options: DPlayerOptions = {
        ...
        contextmenu: [
            ...
            },
            {
                ...
                click: async () => {
                    if(isNaN(Number(aid))){
                        useTipMessageShort("直播间不支持此功能", "warning")
                        return
                    }
...
onMounted(
    async () => {
        ...
        if (Number(route.query.dash) === 1) {
            // todo-阿b实际已经取消flv流了, 不再需要flv.js, 变量名称待修改
            flv = false
        }
        console.log("flvMode: ", flv)
