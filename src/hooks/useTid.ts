export const tidRelation: any = {
    tagSort: {
        ...
        "汽车": ["汽车生活", "汽车文化", "汽车极客", "摩托车", "智能出行", "购车攻略", "改装玩车", "新能源车", "赛车"], // 220805-"新能源车", 220926-"赛车"
        "广告": ["广告"],//广告区消失
    },
    tagIndex: {
        ...
        "汽车": [176, 224, 225, 240, 226, 227, 246, 247, 245], // 0926-245
        "广告": [166],

    }
}

export const useTagIndex = (tid: number): string | undefined => {
    let obj = tidRelation.tagIndex
    for (const key of Object.keys(obj)) {
        if (obj[key].includes(tid)) {
            return key
        }
    }
    return undefined
...
export const generateTagList = (state: any) => {
    ...
    state.oriList.forEach((item: any) => {
        const tag = useTagIndex(item.tid)
        if (tag) {
            tagSet.push(tag)
        } else {
    ...
    state.tagList = Array.from(new Set(tagSet))// set 唯一性提取
